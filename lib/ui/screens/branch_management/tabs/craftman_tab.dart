...
class CraftManTab extends StatefulWidget {
  const CraftManTab({Key? key}) : super(key: key);

  @override
  State<CraftManTab> createState() => _CraftManTabState();
}

class _CraftManTabState extends State<CraftManTab> {
  // for craftsman
    ...
    try {
      if (!isCraftsmanListFetchingData) {
        ...
        await homeRepository
            .fetchCraftsmanList(branchId: Storage.getValue(FbConstant.branchId))
            .then((list) {
          if (list.isNotEmpty) {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                InkWell(
                  onTap: () {
                    // Add Craftsman Screen
                    AppUtils.navigateTo(
                      context,
                      CustomPageTransition(
                        MyApp.myAppKey,
                        AddCraftsManScreen.routeName,
                      ),
                    ).then((value) {
                      if (value == true) {
                        getCraftsmanList();
  ...
  onSearchTextChanged(String text) async {
    ...
    //           craftsmanDetail.model.serviceInfoModel.selectedServiceNames !=
    //               null
    //       ? craftsmanDetail.model.serviceInfoModel.selectedServiceNames!.any(
    //           (serviceName) =>
    //               serviceName.contains(text) ||
    //               serviceName.toLowerCase().contains(text.toLowerCase()))
    ...
    // }
    for (var craftsmanDetail in craftsmanList) {
      if (craftsmanDetail.model.personalDetailsModel.name
              .toLowerCase()
              .contains(text.toLowerCase()) ||
          (craftsmanDetail.model.serviceInfoModel.selectedServiceNames !=
                      null &&
                  craftsmanDetail
                      .model.serviceInfoModel.selectedServiceNames!.isNotEmpty
              ? craftsmanDetail.model.serviceInfoModel.selectedServiceNames!
                  .any((serviceName) =>
                      serviceName.toLowerCase().contains(text.toLowerCase()))
              : craftsmanDetail.model.serviceInfoModel.serviceType
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            SizedBox(height: 5.sp),
            craftManCardRowText(
                key: customCraftsmanModel
                                .model.serviceInfoModel.selectedServiceNames !=
                            null &&
                        customCraftsmanModel.model.serviceInfoModel
            ...
            craftManCardRowText(
                key:
                    'Working Capacity : ${customCraftsmanModel.model.serviceInfoModel.workCapacity.toString()}',
